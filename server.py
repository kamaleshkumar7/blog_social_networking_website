...
def index():
    ...
    if 'userid' in session and 'username' in session:
        ...
        return render_template('home.html',userid=useridv,username= usernamev,posts=result1,inPostPage=0)
    else:
        result1 = gather_data1(session_id=0)
        print('inside session not found')
        app.secret_key = secretkeygeneration()
        return render_template('home.html',userid=0,username='none',posts=result1,inPostPage=0)
...
def myposts():
    ...
    try:       
        ...
        return render_template('home.html',userid=useridv,username= usernamev,posts=result1,inPostPage=1,stat1=stat1data,stat2=stat2data)
    except:
        return redirect(url_for('index'))
...
def search():
   if request.method == 'POST':
       ...
       if 'userid' in session and 'username' in session:
            ...
            if not result1:
                return redirect(url_for('index'))
            return render_template('home.html',userid=useridv,username= usernamev,posts=result1,inPostPage=2)
       else:
            result1 = gather_data1(session_id=0,search_key=search_key)
            if not result1:
                return redirect(url_for('index'))
            return render_template('home.html',userid=0,username='none',posts=result1,inPostPage=2)
